package Aspect::JoinPoint::Return;

# $Id: Return.pm,v 1.3 2002/07/31 21:29:18 marcelgr Exp $
#
# $Log: Return.pm,v $
...
sub install {
	my ($self, $code) = @_;
	my $w1 = wrap $self->sub(), post => sub { $::thisjp = $self };
	my $w2 = wrap $self->sub(), post => $code;
...
saying that it is a return join point and also gives the name of
the subroutine this join point belongs to.

=item C<install(coderef)>

This method takes a coderef (to advice code) and installs it at a
location appropriate to this call join point, i.e. as a postwrapper
...
caller and wantarray information are preserved and that the wrapped
subroutine's return value can be overridden.
