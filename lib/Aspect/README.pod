...
=head1 CHANGES IN VERSION 0.08

=over 4

=item *

The distribution has been adapted for use with perl5.8.0. C<Hook::LexWrap>
is now a core module, but there was a problem with context (wantarray)
handling with Aspect::Memoize.

Sorry about the perl 5.6.x users, but this is bleeding-edge stuff, so
you're expected to use the very latest gear. Anyway, you can still get an
earlier version (up to 0.07) from CPAN, which will work with perl 5.6.x.

=item *

C<Aspect::Memoize> is now intended for subroutines returning scalars
only. That is, if the sub is called in list context, it will still only
